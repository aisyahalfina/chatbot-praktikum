...
async function testModel(apiKey, model) {
    ...
    try {
        ...
        return {
            ok: false, status: res.status, error: err?.error?.message || `HTTP ${res.status}`
        };
    } catch (err) {
...
async function main() {
    ...
    console.log("\n" + BOLD + "[4/4] Validasi API Key & Model" + RESET); if (!apiKey) {
        ...
        for (const model of MODELS) {
            ...
            if (result.ok) {
                ...
                const shortErr = result.error.length > 60 ? result.error.substring(0, 60) + "..." :
                    result.error;
